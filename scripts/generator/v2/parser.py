...
class ClausewitzParser:
    ...
    def _consume_key(self) -> str:
        token = self._current()
        if token.type not in {TokenType.IDENTIFIER, TokenType.KEYWORD, TokenType.MODIFIER, TokenType.TRIGGER}:
            raise ValueError(f"Expected identifier, got {token.type}")
        self.index += 1
        value = token.value
        if not isinstance(value, str):
            raise TypeError("Identifier tokens must carry string values")
        return value
